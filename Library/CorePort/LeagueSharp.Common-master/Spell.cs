namespace LeagueSharp.Common
{
    ...
    public class Spell
    {
        ...
        public float Cooldown
        {
            get
            {
                var coolDown = Player.GetSpell(this.Slot).CooldownExpires;
        ...
        public bool IsCharging
        {
            get
            {
                ...
                return ObjectManager.Player.HasBuff(this.ChargedBuffName) || Utils.TickCount - this._chargedCastedT < 300 + Game.Ping;
            }
        }
        ...
        public float ManaCost
        {
            get
            {
                return ObjectManager.Player.Spellbook.GetSpell(this.Slot).SData.Mana;
            }
        }
        ...
        public void SetCharged(string spellName, string buffName, int minRange, int maxRange, float deltaT)
        {
            ...
            Spellbook.OnCastSpell += new SpellbookCastSpell(this.SpellbookOnCastSpell);
            Spellbook.OnUpdateChargeableSpell += new SpellbookUpdateChargeableSpell(this.Spellbook_OnUpdateChargedSpell);
        ...
        public void Setinterruptible(bool letUserCancel, bool targetted, bool letSpellCancel = false)
        {
            ...
            Obj_AI_Base.OnSpellCast += new Obj_AI_BaseDoCastSpell(this.OnDoCast);
            GameObject.OnDelete += new GameObjectDelete(this.OnDelete);
            Game.OnWndProc += new GameWndProc(this.OnWndProc);
            Player.OnIssueOrder += new Player_ProcessIssueOrder(this.OnOrder);
            Spellbook.OnCastSpell += new SpellbookCastSpell(this.OnCastSpell);

            if (ObjectManager.Player.ChampionName == "Fiddlesticks")
            {
                GameObject.OnCreate += new GameObjectCreate(this.OnCreate);
            }
        }
            ...
            int minTargets = -1)
        {
            ...
            if (prediction.Hitchance < this.MinHitChance || (exactHitChance && prediction.Hitchance != this.MinHitChance))
            {
