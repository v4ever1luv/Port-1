namespace LeagueSharp.Common
{
    ...
    public static class Geometry
    {
        ...
        {
            using (IEnumerator<SpellDataInst> enumerator = (from spell in target.Spellbook.Spells
                                                            where string.Equals(spell.Name, spellName, StringComparison.CurrentCultureIgnoreCase)
                                                            select spell).GetEnumerator())
            {
                if (enumerator.MoveNext())
                {
                    SpellDataInst current = enumerator.Current;
                    return current.Slot;
                }
            }
