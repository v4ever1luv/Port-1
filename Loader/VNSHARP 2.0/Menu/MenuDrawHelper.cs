namespace VNSHARP
{
    ...
    internal static class MenuDrawHelper
    {
        ...
        internal static void DrawBox(
            ...
            Color borderColor)
        {
            cttbotfix.DrawLine(position.X, position.Y + 16, position.X + width, position.Y + 16, height, color);
            //Menu.tabmenuMain.Draw(new Vector2(position.X, position.Y), new Rectangle(0, 0, width, height));

            if (borderwidth > 0)
            {
                cttbotfix.DrawLine(position.X, position.Y, position.X + width, position.Y, borderwidth, borderColor);
                cttbotfix.DrawLine(position.X, position.Y + height, position.X + width, position.Y + height,borderwidth, borderColor);
                cttbotfix.DrawLine(position.X, position.Y, position.X, position.Y + height, borderwidth, borderColor);
                cttbotfix.DrawLine(position.X + width, position.Y, position.X + width, position.Y + height, borderwidth, borderColor);
            }
        }
        ...
        internal static void DrawOnOff(bool on, Vector2 position, MenuItem item)
        {
            ...
            Font.DrawText(
                ...
                new Rectangle(
                    ...
                    item.Height),
                FontDrawFlags.VerticalCenter | FontDrawFlags.Center,
                on ? new ColorBGRA(0, 0, 0, 255) : new ColorBGRA(255, 255, 255, 255));
        }
            ...
            bool drawText)
        {
            ...
            cttbotfix.DrawLine(x - 2, position.Y , x - 2, position.Y + item.Height, 2, Color.Red);
        ...
        internal static void DrawToolTipText(Vector2 position, MenuItem item, SharpDX.Color? textColor = null)
        {
            ...
            Font.DrawText(
                ...
                new Rectangle(
                    ...
                    item.Height + 8),
                FontDrawFlags.VerticalCenter,
                textColor ?? SharpDX.Color.Yellow);
        }
