...
namespace SFXUtility.Features.Trackers
{
    internal class Sidebar : Child<Trackers>
    {
        ...
        private void OnDrawingEndScene(EventArgs args)
        {
            try
            {
                ...
                foreach (var enemy in _enemyObjects)
                {
                    ...
                    if (enemy.TeleportStatus == TeleportStatus.Start ||
                        (enemy.TeleportStatus == TeleportStatus.Finish ||
                         enemy.TeleportStatus == TeleportStatus.Abort) &&
                        Game.Time <= enemy.LastTeleportStatusTime + 5f)
                    {
                        _sprite.Begin(SpriteFlags.AlphaBlend);
                        _sprite.DrawCentered(
                            enemy.TeleportStatus == TeleportStatus.Start
                ...
                if (enemyObject != null)
                {
                    if (args.Type == TeleportType.Teleport &&
                        (args.Status == TeleportStatus.Finish ||
                         args.Status == TeleportStatus.Abort))
                    {
                        var time = Game.Time;
                    ...
                    args.Msg == (uint)WindowsMessages.WM_LBUTTONDBLCLK)
                {
                    ...
                    foreach (var enemy in _enemyObjects)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                EloBuddy.Player.IssueOrder(GameObjectOrder.MoveTo, enemy.Unit);
                            }
                        }
                        index++;
                    }
        ...
        {
            private TeleportStatus _teleportStatus;

            public EnemyObject(AIHeroClient unit, Texture texture)
            {
                TeleportStatus = TeleportStatus.Unknown;
                Unit = unit;
                Texture = texture;
            ...
            public AIHeroClient Unit { get; private set; }
            ...
            public TeleportStatus TeleportStatus
            {
