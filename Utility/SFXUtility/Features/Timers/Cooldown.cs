    ...
    internal class Cooldown : Child<Timers>
    {
        ...
        private void OnDrawingEndScene(EventArgs args)
        {
            try
            {
                ...
                foreach (var hero in _heroes.Where(hero => hero != null && hero.IsValid && hero.IsHPBarRendered && hero.Position.IsOnScreen()))
                {
                    try
                    {
                        var lHero = hero;
                        ...
                        var x = (int)hero.HPBarPosition.X - (hero.IsMe ? 2 : 15);
                        var y = (int)hero.HPBarPosition.Y + (hero.IsEnemy ? 3 : (hero.IsMe ? -3 : 1));
