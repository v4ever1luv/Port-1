...
namespace SFXUtility.Features.Drawings
{
    internal class PerfectWard : Child<Drawings>
    {
        ...
        protected override List<LeagueSharp.Common.Utility.Map.MapType> BlacklistedMaps
        {
            get
            {
                return new List<LeagueSharp.Common.Utility.Map.MapType>
                {
                    LeagueSharp.Common.Utility.Map.MapType.CrystalScar,
                    LeagueSharp.Common.Utility.Map.MapType.HowlingAbyss,
        ...
        private void OnGameUpdate(EventArgs args)
        {
            try
            {
                ...
                if (Menu.Item(Name + "Hotkey").IsActive() || Menu.Item(Name + "PermaShow").GetValue<bool>())
                {
                    ...
                    {
                        EloBuddy.Player.IssueOrder(GameObjectOrder.MoveTo, _lastWardSpot.MovePosition);
                    }
                }
                else
                {
                ...
                if (spot.SafeSpot)
                {
                        ...
                        Menu.Item(Name + "DrawingRadius").GetValue<Slider>().Value)
                    {
                        ...
                        if (_lastWardSpot.Equals(default(WardSpot)))
                        {
                            ...
                            EloBuddy.Player.IssueOrder(GameObjectOrder.MoveTo, spot.MovePosition);
                        }
                    }
                }
                else
