...
namespace SFXUtility.Features.Detectors
{
    internal class Teleport : Child<Detectors>
    {
            ...
            try
            {
                ...
                try
                {
                    if (Menu.Item(Name + "DrawingTextEnabled").GetValue<bool>())
                    {
                        ...
                        foreach (var teleport in
                            _teleportObjects.Where(
                                t => t.LastStatus != TeleportStatus.Unknown && t.Update()))
                        {
                    ...
                    if (Menu.Item(Name + "DrawingBarEnabled").GetValue<bool>())
                    {
                        ...
                        var teleports =
                            _teleportObjects.Where(
                                t => t.LastStatus != TeleportStatus.Unknown && t.Update(true))
        ...
        private class TeleportObject
        {
            ...
            public TeleportStatus LastStatus
            {
            ...
            public TeleportType LastType { get; set; }

            public float Countdown
            {
                get
                {
                    if (Hero.IsMe && LastStatus == TeleportStatus.Finish) {}
                    switch (LastStatus)
                    {
            ...
            {
                var additional = LastStatus == TeleportStatus.Start
    ...
    {
        private readonly TeleportStatus _status;
