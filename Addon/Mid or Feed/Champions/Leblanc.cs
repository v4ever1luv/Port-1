...
namespace Mid_or_Feed.Champions
{
    ...
    internal class Leblanc : Plugin
    {
        ...
        public Leblanc()
        {
            ...
            //EloBuddy.Player.OnIssueOrder += ObjAiBaseOnOnIssueOrder;
        ...
        /// <param name="target">The target.</param>
        /// <returns></returns>
        public bool HasQBuff(AIHeroClient target)
        {
                        ...
                        () =>
                            {
                                ...
                                if (clone != null)
                                {
                                    EloBuddy.Player.IssueOrder(GameObjectOrder.MovePet, moveTo);
                                }
                            });
                    break;
        ...
        private void ObjAiBaseOnOnIssueOrder(Obj_AI_Base sender, PlayerIssueOrderEventArgs args)
        {
            ...
            Chat.Print("Order: {0} Converted: {1}", args.Order.ToString(), convertedOrder.ToString());

            var clone = this.Player.Pet as Obj_AI_Base;
            if (clone != null && this.HasValidClone)
            {
                EloBuddy.Player.IssueOrder(convertedOrder, args.Target);
                EloBuddy.Player.IssueOrder(convertedOrder, args.TargetPosition);
            }
        }
