    ...
    public class ComboProvider
    {
        ...
        private void OnLevelUp(Obj_AI_Base sender, EventArgs args)
        {
            ...
            if (_autoLevelRndDelay)
                LeagueSharp.Common.Utility.DelayAction.Add(_random.Next(100, 500), CallbackLevelUp);
            else
                CallbackLevelUp();
        }
        ...
        protected virtual void OnUpdate(Orbwalking.OrbwalkingMode mode)
        {
            ...
            if (mode != Orbwalking.OrbwalkingMode.Combo && ObjectManager.Player.Spellbook.IsAutoAttacking) return;
