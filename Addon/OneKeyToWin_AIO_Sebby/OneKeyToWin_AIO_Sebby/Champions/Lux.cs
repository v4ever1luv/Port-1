    ...
    class Lux : Base
    {
        ...
        private void LogicQ()
        {
            ...
            if (t.IsValidTarget() && Config.Item("Qon" + t.ChampionName).GetValue<bool>())
            {
                if (Program.Combo && Player.Mana > RMANA + QMANA)
                    CastQ(t);
                else if (Program.Farm  && Config.Item("harassQ", true).GetValue<bool>() && Config.Item("Harass" + t.ChampionName).GetValue<bool>() && Player.Mana > RMANA + EMANA + WMANA + EMANA)
                    CastQ(t);
            ...
            else
            {
                ...
                if (t.IsValidTarget() )
                {
                    if (!Config.Item("autoEcc", true).GetValue<bool>())
                    {
                        ...
                        else if (Program.Farm && OktwCommon.CanHarras() && Config.Item("harassE", true).GetValue<bool>() && Player.Mana > RMANA + EMANA + EMANA + RMANA)
                            Program.CastSpell(E, t);
        ...
        private void KsJungle()
        {
            ...
            foreach (var mob in mobs)
            {
                ...
                if (((mob.BaseSkinName == "SRU_Dragon" && Config.Item("Rdragon", true).GetValue<bool>())
