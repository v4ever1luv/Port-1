...
namespace OneKeyToWin_AIO_Sebby.Champions
{
    class Jinx : Base
    {
        ...
        private void LogicQ()
        {
            if (Program.Farm && !FishBoneActive && !Player.Spellbook.IsAutoAttacking && Orbwalker.GetTarget() == null && Orbwalking.CanAttack() && Config.Item("farmQout", true).GetValue<bool>() && Player.Mana > RMANA + WMANA + EMANA + 10)
            {
            ...
            if (t.IsValidTarget())
            {
                if (!FishBoneActive && (!Orbwalking.InAutoAttackRange(t) || t.CountEnemiesInRange(250) > 2) && Orbwalker.GetTarget() == null)
                {
                    ...
                    if (Program.Combo && (Player.Mana > RMANA + WMANA + 10 || Player.GetAutoAttackDamage(t) * 3 > t.Health))
                        Q.Cast();
                    else if (Program.Farm && !Player.Spellbook.IsAutoAttacking && Orbwalking.CanAttack() && Config.Item("Qharras", true).GetValue<bool>() && !ObjectManager.Player.UnderTurret(true) && Player.Mana > RMANA + WMANA + EMANA + 20 && distance < bonusRange() + t.BoundingRadius + Player.BoundingRadius)
                        Q.Cast();
                }
            }
        ...
        private void KsJungle()
        {
            ...
            foreach (var mob in mobs)
            {
                ...
                if (mob.Health < mob.MaxHealth && ((mob.BaseSkinName.ToLower().Contains("dragon") && Config.Item("Rdragon", true).GetValue<bool>())
