    ...
    internal class Yasuo : Helper
    {
                ...
                else
                {
                    ...
                    if (minionObj.Any())
                    {
                        if (!HaveQ3)
                        {
                            ...
                            if (obj != null && Q.Cast(obj).IsCasted())
                            {
                                return;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        if (target != null)
                        {
                            ...
                            else
                            {
                                ...
                                else if (Q2.Cast(target).IsCasted())
                                {
                                    return;
                                }
                            }
            ...
            if (GetValue<bool>("KillSteal", "Q") && Q.IsReady())
            {
                ...
                else
                {
                    ...
                    if (target != null && CanKill(target, GetQDmg(target))
                        && (!HaveQ3 ? Q : Q2).Cast(target).IsCasted())
                    {
                        return;
                    }
                }
        ...
        private static void LastHit()
        {
                ...
                && (!HaveQ3 || GetValue<bool>("LastHit", "Q3")))
            {
                ...
                if (obj != null && (!HaveQ3 ? Q : Q2).Cast(obj).IsCasted())
                {
                    return;
                }
            }
                ...
                var obj =
                    GetMinions(E.Range, MinionTypes.All, MinionTeam.NotAlly, MinionOrderTypes.MaxHealth)
                        .Where(
                            i =>
                            CanCastE(i)
                            && (!Orbwalking.InAutoAttackRange(i) || i.Health > Player.GetAutoAttackDamage(i, true))
        ...
        private static void OnPossibleToInterrupt(AIHeroClient unit, InterruptableSpell spell)
        {
            if (Player.IsDead || !GetValue<bool>("Interrupt", "Q")
                || !GetValue<bool>("Interrupt", unit.ChampionName + "_" + spell) || !HaveQ3)
            {
                return;
            }
        ...
        private static void StackQ()
        {
            ...
            else
            {
                ...
                if (obj != null)
                {
                    Q.CastIfHitchanceEquals(obj, HitChance.Medium);
                }
            }
        }
        ...
        protected class EvadeTarget
        {
            ...
            private static void ObjSpellMissileOnCreate(GameObject sender, EventArgs args)
            {
                ...
                if (!missile.SpellCaster.IsValid<AIHeroClient>() || missile.SpellCaster.Team == Player.Team)
                {
                    return;
                }
