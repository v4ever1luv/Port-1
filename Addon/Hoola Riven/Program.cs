...
namespace HoolaRiven
{
    public class Program
    {
        ...
        private static void OnGameLoad(EventArgs args)
        {
            ...
            Obj_AI_Base.OnSpellCast += new Obj_AI_BaseDoCastSpell(OnDoCast);
        ...
        private static void OnMenuLoad()
        {
            ...
            LeagueSharp.Common.TargetSelector.AddToMenu(ts);
        ...
        /*
        public static bool InWRange(GameObject target)
        {
            if (target != null)
            {
                return (Player.HasBuff("RivenFengShuiEngine") && target != null) ? 330 >= Player.Distance(target.Position) : 265 >= Player.Distance(target.Position);
            }
            else return false;
        }//*/
        ...
        private static double Rdame(Obj_AI_Base target, double health)
        {
            if (target != null)
            {
                ...
                return Player.CalcDamage(target, LeagueSharp.Common.Damage.DamageType.Magical, rawdmg * (1 + pluspercent));
            }
            return 0;
        }
    }
