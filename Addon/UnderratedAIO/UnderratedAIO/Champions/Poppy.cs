...
namespace UnderratedAIO.Champions
{
    internal class Poppy
    {
        ...
        public static float ComboDamage(AIHeroClient hero)
        {
            ...
            if ((Items.HasItem(ItemHandler.Bft.Id) && Items.CanUseItem(ItemHandler.Bft.Id)) || (Items.HasItem(ItemHandler.Dfg.Id) && Items.CanUseItem(ItemHandler.Dfg.Id)))
            {
                damage = (float) (damage * 1.2);
            }
            
            if (player.Spellbook.CanUseSpell(player.GetSpellSlot("summonerdot")) == SpellState.Ready && player.Distance(hero) < 500)
            {
